...
class SchedulerStats:
    ...
    # Beam KV usage normalized to tokens. These are comparable with the
    # scheduler-wide KV pool token metrics even when the allocator is paged.
    beam_kv_live_tokens: int = 0
...
class SchedulerMetricsCollector(_StatLoggerDIMixin):
    ...
    def __init__(
        ...
        self.beam_kv_live_tokens = Gauge(
            name="sglang:beam_kv_live_tokens",
            documentation="Physical KV tokens currently owned by trie beam requests.",
            labelnames=labels.keys(),
            multiprocess_mode="mostrecent",
        )
        self.beam_kv_live_reference_tokens = Gauge(
            name="sglang:beam_kv_live_reference_tokens",
            documentation="Logical trie beam KV tokens without prefix sharing.",
            labelnames=labels.keys(),
            multiprocess_mode="mostrecent",
        )
        self.beam_kv_shared_tokens = Gauge(
            name="sglang:beam_kv_shared_tokens",
            documentation="Trie beam logical KV tokens avoided through prefix sharing.",
            labelnames=labels.keys(),
            multiprocess_mode="mostrecent",
        )
    ...
    def log_stats(self, stats: SchedulerStats) -> None:
        ...
        self._log_gauge(self.beam_kv_live_tokens, stats.beam_kv_live_tokens)
        self._log_gauge(
            self.beam_kv_live_reference_tokens,
            stats.beam_kv_live_reference_tokens,
        )
